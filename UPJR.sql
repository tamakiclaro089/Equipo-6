...
create table carrera(idCarrera varchar(3) primary key not null,
...
select * from carrera
select * from materia
where alumno='324030027'

/*24/07/25*/

/*Hacer una consulta que permita 
cargar (asignar) todas las materias que
correspondan a los y las alumnas
de ITR, para el periodo Septiembre Diciembre 2024*/
...
/*Hacer una consulta consideracion 
la siguiente informacion: 
Todos los alumnos y alumnas que llevaron 
Fundamentos matematicos obtuvieron 6 de 
calificación*/

update historial
set calif=6
from historial join materia 
on historial.materia = materia.idMateria
where 
materia.nomMateria= 'Fundamentos matematicos'
select * from historial

/*El desarrollador de la BD se dio cuenta 
de las materias son cursadas en diferentes 
tipo de cursamiento:
1-Ordinario
2-Recursamiento
entonces necesita generar una actualizacion
para que la informacion exixtente contenga
el tipo de cursamiento.
Nota: todas las materias del periodo
septiembre diciembre fueron ordinarios*/

create table TipoCurso
(idTC int primary key not null,
nomTC varchar (15))

drop table 
insert into TipoCurso
(idTC,nomTC)
values
(1,'Ordinario'),
(2,'Recursamiento')
...
/*Las personas que tuvieron materias reprobadas 
el periodo septiembre diciembre 2024,
cargaron nuevamente la materia durante el cuatrimestre 
enero abril 2025 y la acreditaron con 8*/

create table materias_reprobadas 
(idMR int primary key not null,
nomMR varchar (20))

insert into historial
(alumno,materia,calif, periodo,curso)
select alumno, materia, 8, 2501,2
from historial
where periodo = 2403 and calif = 6
